...
type ContactChannel = 'instagram' | 'telegram' | 'whatsapp'

export default function FloatingContact() {
  const [hovered, setHovered] = useState<ContactChannel | null>(null)

  const whatsappUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(WHATSAPP_MESSAGE)}`
  const telegramUrl = `https://t.me/${TELEGRAM_USERNAME}`

  const labelStyle = (channel: ContactChannel) => ({
    whiteSpace: 'nowrap' as const,
    opacity: hovered === channel ? 1 : 0,
  ...
  return (
    <div className="fixed bottom-6 right-5 flex flex-col gap-3 z-50">
      ...
      <a
        ...
        aria-label="Instagram orqali kuzating"
        ...
        style={{
          ...
          overflow: 'hidden',
          maxWidth: hovered === 'instagram' ? '180px' : '52px',
          transform: hovered === 'instagram' ? 'translateY(-2px)' : 'translateY(0)',
        }}
      >
        <svg
        ...
        <span style={labelStyle('instagram')}>Instagram</span>
      </a>
      ...
      >
        ...
        <span style={labelStyle('telegram')}>Telegram</span>
      </a>
      ...
      >
        ...
        <span style={labelStyle('whatsapp')}>WhatsApp</span>
      </a>
    </div>
  )
}
